...
def evaluate_vehicle_depot_pair(args):
    """
    Worker function to evaluate a single vehicle-depot pair.
    This function will be executed in parallel processes.
    """
    (vehicle_idx, start_depot, end_depot, G_edges, G_nodes, vehicle_capacity, 
     remaining_required_edges, current_route_time) = args
    
    try:
        # Reconstruct graph from edges and nodes (since Graph objects can't be pickled easily)
        import networkx as nx
        G = nx.Graph()
        ...
        from magnetic_field import IntelligentCapacityTuner
        
        # Create tuner for this vehicle-depot pair
        ...
        return {'vehicle_idx': vehicle_idx, 'success': False}
        
    except Exception as e:
        return {'vehicle_idx': vehicle_idx, 'success': False, 'error': str(e)}

def find_best_assignment_parallel(G, non_failed_vehicles, vehicle_end_locations, 
                                depot_nodes, depot_distances, vehicle_capacity, 
                                remaining_required_edges, vehicle_route_times, 
                                recharge_time, num_processes=None):
    """
    Parallelized version of finding the best vehicle assignment.
    Uses multiprocessing to evaluate vehicle-depot combinations in parallel.
    """
    if num_processes is None:
        num_processes = min(mp.cpu_count(), len(non_failed_vehicles) * len(depot_nodes))
    
    # Prepare data for multiprocessing (convert graph to serializable format)
    G_edges = [(u, v, d['weight']) for u, v, d in G.edges(data=True)]
    G_nodes = list(G.nodes())
    
    # Create all vehicle-depot combinations to evaluate
    tasks = []
    for vehicle_idx in non_failed_vehicles:
        ...
        reachable_depots = find_reachable_depots(G, start_depot, depot_nodes, 
                                               vehicle_capacity, depot_distances)
        
        # Create tasks for each reachable depot
        for end_depot in reachable_depots:
            task = (vehicle_idx, start_depot, end_depot, G_edges, G_nodes, 
                   vehicle_capacity, remaining_required_edges, current_route_time)
            tasks.append(task)
    ...
    with mp.Pool(processes=num_processes) as pool:
        results = pool.map(evaluate_vehicle_depot_pair, tasks)
    ...
    # Find the best assignment among all successful results
    # Sort by coverage (descending), then by cost (ascending)
    successful_results.sort(key=lambda x: (-x['coverage'], x['cost']))
    
    best_result = successful_results[0]
    
    # Convert to expected format
    return {
        'vehicle_idx': best_result['vehicle_idx'],
        'start_depot': best_result['start_depot'],
        'end_depot': best_result['end_depot'],
        'trip': best_result['trip'],
        'cost': best_result['cost'],
        'coverage': best_result['coverage'],
        'route_time_increase': best_result['cost']
    }
...
def handle_vehicle_failure_with_magnetic_field(G, t, failure_history, vehicle_routes, vehicle_trip_times, 
                                              ...
                                              num_processes=None):
    """
    Parallelized version of vehicle failure handling using magnetic field approach.
    ...
    print(f"Starting parallelized magnetic field failure handling at time {t}")
    ...
    print(f"Using {num_processes or mp.cpu_count()} processes for parallel evaluation")
    ...
    while remaining_required_edges:
        iteration += 1
        print(f"\nIteration {iteration}: {len(remaining_required_edges)} edges remaining")
        
        # PARALLEL SECTION: Find the best assignment among all vehicles
        best_assignment = find_best_assignment_parallel(
            G, non_failed_vehicles, vehicle_end_locations, depot_nodes, 
            depot_distances, vehicle_capacity, remaining_required_edges, 
            vehicle_route_times, recharge_time, num_processes
        )
        ...
        print(f"Selected vehicle {vehicle_idx}")
        print(f"  Trip: {trip}")
        print(f"  Coverage: {coverage}, Cost: {cost:.2f}")
        ...
        # Update vehicle end location and route time
        vehicle_end_locations[vehicle_idx] = trip[-1]
        vehicle_route_times[vehicle_idx] += cost + recharge_time
        
        print(f"Removed {len(edges_to_remove)} edges. Remaining: {len(remaining_required_edges)}")
    
    execution_time = time.time() - start_time
    
    print(f"\nParallelized magnetic field failure handling completed in {execution_time:.3f} seconds")
    print(f"Remaining uncovered edges: {len(remaining_required_edges)}")
    print(f"Total iterations: {iteration}")
    
    # Create metrics similar to original results
    mf_results = {
