...
def handle_vehicle_failure_with_improved_magnetic_field(G, t, failure_history, vehicle_routes, vehicle_trip_times, 
    ...
    iteration = 0
    while remaining_required_edges and iteration < 20:  # Prevent infinite loops
        iteration += 1
        ...
        combinations = evaluate_vehicle_depot_combinations(
            G, non_failed_vehicles, vehicle_end_locations, depot_nodes, 
            depot_distances, vehicle_capacity, remaining_required_edges, vehicle_route_times,
            recharge_time
        )
        
        if not combinations:
            print("[IMPROVED MF] No valid combinations found")
            break
        
        print(f"[IMPROVED MF] Found {len(combinations)} valid combinations")
        
        # Display top 5 combinations for debugging
        print("[IMPROVED MF] Top 5 combinations:")
        for i, combo in enumerate(combinations[:5]):
            print(f"  {i+1}. Vehicle {combo['vehicle_idx']}: {combo['start_depot']}->{combo['end_depot']}, "
                  f"Coverage: {combo['coverage']}, Efficiency: {combo['coverage_efficiency']:.3f}, "
                  f"Score: {combo['combined_score']:.3f}")
        
        # Select best combination
        best_combo = combinations[0]
        vehicle_idx = best_combo['vehicle_idx']
        
        # Assign the best trip
        print(f"[IMPROVED MF] Assigning trip to vehicle {vehicle_idx}")
        print(f"[IMPROVED MF]   Trip: {best_combo['trip']}")
        print(f"[IMPROVED MF]   Coverage: {best_combo['coverage']}, Cost: {best_combo['cost']:.2f}")
        
        # Add trip to vehicle
        vehicle_routes[vehicle_idx].append(best_combo['trip'])
        vehicle_trip_times[vehicle_idx].append(best_combo['cost'])
        new_trips_assigned[vehicle_idx].append(best_combo['trip'])
        
        # Remove covered edges
        edges_to_remove = []
        for i in range(len(best_combo['trip']) - 1):
            trip_edge = [best_combo['trip'][i], best_combo['trip'][i + 1]]
            trip_edge_reversed = [best_combo['trip'][i + 1], best_combo['trip'][i]]
        ...
        vehicle_end_locations[vehicle_idx] = best_combo['trip'][-1]
        vehicle_route_times[vehicle_idx] += best_combo['cost']
        
        print(f"[IMPROVED MF] Removed {len(edges_to_remove)} edges. Remaining: {len(remaining_required_edges)}")
